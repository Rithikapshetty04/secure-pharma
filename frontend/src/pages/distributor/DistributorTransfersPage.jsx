...
export default function DistributorTransfersPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    location: user?.organization?.address || 'Regional Wholesale Distribution Center #1',
    notes: '',
  });
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (batchesRes.success) {
          // Valid batches for transfer (not expired, not recalled)
        ...
        console.error('Error fetching distributor transfer prerequisites:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.toOrganizationId) {
      setError('Please select an approved pharmacy.');
      return;
    }
    ...
    if (new Date(selectedBatch.expiryDate) <= new Date()) {
      setError('Expired batches cannot be transferred to pharmacies.');
      return;
    }
    ...
    try {
      const payload = {
        ...
        quantity: qty,
        location: formData.location.trim() || 'Logistics Transfer Bay',
        notes: formData.notes.trim() || `Wholesale distribution delivery to licensed pharmacy on ${formData.transferDate}.`,
      };

      const res = await api.recordEvent(payload);

      if (res.success) {
        setSuccessMsg(`✓ Transfer to pharmacy recorded successfully! Redirecting...`);
        setTimeout(() => {
          navigate(`/distributor/batches/${formData.batchId}`);
        }, 1200);
      } else {
      ...
      <div className="glass-card" style={{ padding: '36px', background: '#ffffff' }}>
        <div style={{ marginBottom: '24px' }}>
          <h1 style={{ fontSize: '1.6rem', fontWeight: '800', margin: 0, color: '#1e293b' }}>
            🚚 Transfer Batch to Approved Pharmacy
          </h1>
          <p style={{ color: 'var(--text-dim)', fontSize: '0.85rem', marginTop: '4px' }}>
            Forward medicine custody from wholesale distribution inventory to a verified retail or hospital pharmacy.
          </p>
        </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '40px 0', color: '#2563eb' }}>
            Loading transfer parameters...
          </div>
        ) : batches.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-dim)' }}>
            No transferable batches available in inventory.
          </div>
        ) : (
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {selectedBatchInfo && (
              ...
              >
                ...
                <div>
                  <span style={{ color: 'var(--text-dim)', fontSize: '0.75rem', display: 'block' }}>EXPIRY:</span>
                  <strong style={{ color: '#059669' }}>
                    {new Date(selectedBatchInfo.expiryDate).toLocaleDateString()}
                  </strong>
                </div>
                <div>
                  <span style={{ color: 'var(--text-dim)', fontSize: '0.75rem', display: 'block' }}>CURRENT STATUS:</span>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: '700', marginBottom: '6px', color: 'var(--text-dim)' }}>
                SELECT APPROVED PHARMACY DESTINATION *
              </label>
              {pharmacies.length === 0 ? (
                <div style={{ color: '#b91c1c', fontSize: '0.85rem', padding: '8px 0' }}>
                  No approved pharmacies found on the regulatory ledger.
                </div>
              ) : (
                <select
                  required
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Regional Distribution Warehouse Hub #1, Chicago IL"
                style={{ width: '100%', padding: '10px 12px', borderRadius: 'var(--radius-sm)', fontSize: '0.85rem' }}
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="e.g. Dispatched to pharmacy for retail inventory fulfillment."
                style={{ width: '100%', padding: '10px 12px', borderRadius: 'var(--radius-sm)', fontSize: '0.85rem' }}
              />
            </div>
            ...
            <button
              type="submit"
              disabled={submitting || pharmacies.length === 0}
              className="btn btn-primary"
              style={{ padding: '14px', fontSize: '1rem', marginTop: '8px' }}
            >
              {submitting ? 'Recording Custody Transfer...' : 'Complete Transfer to Pharmacy'}
            </button>
          </form>
        )}
      </div>
