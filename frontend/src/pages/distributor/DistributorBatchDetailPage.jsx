...
export default function DistributorBatchDetailPage() {
  ...
  const justTransferred = searchParams.get('transferred') === 'true';
  ...
  const loadBatchDetails = async () => {
    ...
    try {
      ...
      if (res.success && res.batch) {
        ...
        setError(res.message || 'Batch not found in distributor records.');
      }
    } catch (err) {
      setError(err.message || 'Error connecting to batch registry.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '30px auto 80px', padding: '0 20px' }}>
      ...
      {justTransferred && (
        ...
        >
          <span>✓</span>
          <span>Custody transfer for Batch #{batch.batchNumber} has been successfully recorded in the supply-chain ledger!</span>
        </div>
      )}
      ...
      <div className="glass-card" style={{ padding: '28px', background: '#ffffff', marginBottom: '24px' }}>
        ...
        >
          ...
          <div>
            <span style={{ color: 'var(--text-dim)', display: 'block', fontSize: '0.75rem', fontWeight: '700' }}>AVAILABLE STOCK</span>
          ...
          <div>
            ...
            <span style={{ color: 'var(--text-muted)' }}>{batch.storageRequirements || 'Controlled room temperature'}</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="glass-card" style={{ padding: '24px', textAlign: 'center', background: '#ffffff' }}>
          ...
          <div style={{ marginTop: '20px', display: 'flex', gap: '10px', justifyContent: 'center' }}>
            ...
            <Link
              to={`/distributor/transfers/${batch._id}`}
